...
class ConfigService:
    def get_data_source(self, market: str) -> DataSource:
        # 根据市场返回最优数据源
\`\`\``,d={"SPEC.md":{name:"架构总览",icon:"🏗️",content:u},"multi-agent-architecture.md":{name:"多智能体架构",icon:"🤖",content:y},"stock-analysis-system.md":{name:"股票分析系统",icon:"📊",content:p},"prompt-template-system.md":{name:"提示词模板系统",icon:"📝",content:h},"data-source-architecture.md":{name:"数据源架构",icon:"🗄️",content:A},"trading-strategy.md":{name:"交易策略",icon:"📈",content:x},"configuration-management.md":{name:"配置管理",icon:"⚙️",content:v}},l=[{name:"Fundamental Analyst",role:"基本面分析",group:"Analysts",icon:"📋"},{name:"Technical Analyst",role:"技术分析",group:"Analysts",icon:"📈"},{name:"Market Analyst",role:"市场分析",group:"Analysts",icon:"🌐"},{name:"Sentiment Analyst",role:"情绪分析",group:"Analysts",icon:"💬"},{name:"Data Researcher",role:"数据研究",group:"Researchers",icon:"🔍"},{name:"News Researcher",role:"新闻研究",group:"Researchers",icon:"📰"},{name:"Bull Debater",role:"多方辩手",group:"Debaters",icon:"🐂"},{name:"Bear Debater",role:"空方辩手",group:"Debaters",icon:"🐻"},{name:"Judge",role:"裁判",group:"Debaters",icon:"⚖️"},{name:"Strategy Manager",role:"策略管理",group:"Managers",icon:"🎯"},{name:"Portfolio Manager",role:"组合管理",group:"Managers",icon:"💼"},{name:"Trader",role:"交易执行",group:"Trader",icon:"💹"}],f={Analysts:{icon:"👥",desc:"4位分析师并行的海量分析",agents:l.filter(a=>a.group==="Analysts")},Researchers:{icon:"🔬",desc:"2位研究员提供数据和新闻支撑",agents:l.filter(a=>a.group==="Researchers")},Debaters:{icon:"⚔️",desc:"3位辩手多方辩论 + 裁判裁决",agents:l.filter(a=>a.group==="Debaters")},Managers:{icon:"📋",desc:"2位经理协调策略与组合",agents:l.filter(a=>a.group==="Managers")},Trader:{icon:"💹",desc:"1位交易员执行交易指令",agents:l.filter(a=>a.group==="Trader")}};(function(){const s=localStorage.getItem("darkMode");(s==="true"||s===null&&window.matchMedia("(prefers-color-scheme: dark)").matches)&&document.documentElement.classList.add("dark")})();function m(){var i;const a=document.documentElement.classList.contains("dark"),o=new URLSearchParams(window.location.search).get("file"),c=document.getElementById("app"),e=Object.entries(d).map(([n,r])=>`<a href="?file=${n}" class="nav-link flex items-center gap-2 px-4 py-2 rounded-lg transition ${o===n?"active":""}">${r.icon} ${r.name}</a>`).join(""),t=a?'<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>':'<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>';if(o&&d[o]){const n=d[o];c.innerHTML=`
      <div class="min-h-screen flex">
        <aside class="aside w-64 border-r flex-shrink-0 overflow-y-auto fixed h-full">
          ...
          <nav class="p-3 space-y-1">
            <a href="?" class="nav-link flex items-center gap-2 px-4 py-2 rounded-lg transition ${o?"":"active"}">🏠 首页</a>
            ${e}
          </nav>
        </aside>
            ...
            <div class="card rounded-xl p-6 mb-8">
              ...
              <div class="space-y-6">
                ${Object.entries(f).map(([n,r])=>`
                  <div>
                    <h3 class="text-base font-medium mb-2 text-secondary">${r.icon} ${n} — ${r.desc}</h3>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                      ${r.agents.map(g=>`
                        <div class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm" style="background:var(--bg-secondary)">
                          <span>${g.icon}</span>
                          <div>
                            <div class="font-medium text-primary">${g.name}</div>
                            <div class="text-muted text-xs">${g.role}</div>
                          </div>
                        </div>
                      `).join("")}
                    </div>
...
Report Generation</pre>
            ...
            <div class="card rounded-xl p-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ${Object.entries(d).filter(([n])=>n!=="SPEC.md").map(([n,r])=>`
                  <a href="?file=${n}" class="flex items-center gap-3 px-4 py-3 rounded-lg transition hover:opacity-80" style="background:var(--bg-secondary)">
                    <span class="text-2xl">${r.icon}</span>
                    <div>
                      <div class="font-medium text-primary">${r.name}</div>
                      <div class="text-muted text-xs">${n}</div>
                    </div>
                  </a>
                `).join("")}
              </div>
    ...
    `;(i=document.getElementById("themeToggle"))==null||i.addEventListener("click",()=>{const n=document.documentElement.classList.toggle("dark");localStorage.setItem("darkMode",n),m()})}m();
