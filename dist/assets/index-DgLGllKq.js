...
Trade Execution + Risk Check
    ↓
Report Generation
\`\`\``},"multi-agent-architecture.md":{name:`多智能体架构`,icon:`🤖`,content:`# 多智能体架构
...
\`\`\``},"stock-analysis-system.md":{name:`股票分析系统`,icon:`📊`,content:`# 股票分析系统
...
\`\`\``},"prompt-template-system.md":{name:`提示词模板系统`,icon:`📝`,content:`# 提示词模板系统
...
\`\`\``},"data-source-architecture.md":{name:`数据源架构`,icon:`🗄️`,content:`# 数据源架构
...
- K线数据: 5分钟过期
- 财务数据: 24小时过期
- 新闻数据: 15分钟过期`},"trading-strategy.md":{name:`交易策略`,icon:`📈`,content:`# 交易策略
...
- 总仓位 ≤ 80%
- 日内止损: -5%
- 单日最大亏损: -10%`},"configuration-management.md":{name:`配置管理`,icon:`⚙️`,content:`# 配置管理
...
class ConfigService:
    def get_data_source(self, market: str) -> DataSource:
        # 根据市场返回最优数据源
\`\`\``}},t=[{name:`Fundamental Analyst`,role:`基本面分析`,group:`Analysts`,icon:`📋`},{name:`Technical Analyst`,role:`技术分析`,group:`Analysts`,icon:`📈`},{name:`Market Analyst`,role:`市场分析`,group:`Analysts`,icon:`🌐`},{name:`Sentiment Analyst`,role:`情绪分析`,group:`Analysts`,icon:`💬`},{name:`Data Researcher`,role:`数据研究`,group:`Researchers`,icon:`🔍`},{name:`News Researcher`,role:`新闻研究`,group:`Researchers`,icon:`📰`},{name:`Bull Debater`,role:`多方辩手`,group:`Debaters`,icon:`🐂`},{name:`Bear Debater`,role:`空方辩手`,group:`Debaters`,icon:`🐻`},{name:`Judge`,role:`裁判`,group:`Debaters`,icon:`⚖️`},{name:`Strategy Manager`,role:`策略管理`,group:`Managers`,icon:`🎯`},{name:`Portfolio Manager`,role:`组合管理`,group:`Managers`,icon:`💼`},{name:`Trader`,role:`交易执行`,group:`Trader`,icon:`💹`}],n={Analysts:{icon:`👥`,desc:`4位分析师并行的海量分析`,agents:t.filter(e=>e.group===`Analysts`)},Researchers:{icon:`🔬`,desc:`2位研究员提供数据和新闻支撑`,agents:t.filter(e=>e.group===`Researchers`)},Debaters:{icon:`⚔️`,desc:`3位辩手多方辩论 + 裁判裁决`,agents:t.filter(e=>e.group===`Debaters`)},Managers:{icon:`📋`,desc:`2位经理协调策略与组合`,agents:t.filter(e=>e.group===`Managers`)},Trader:{icon:`💹`,desc:`1位交易员执行交易指令`,agents:t.filter(e=>e.group===`Trader`)}};function r(){let t=new URLSearchParams(window.location.search).get(`doc`),r=document.getElementById(`app`),i=Object.entries(e).map(([e,n])=>`<a href="?doc=${e}" class="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-gray-200 transition ${t===e?`bg-blue-100 text-blue-700 font-medium`:`text-gray-700`}">${n.icon} ${n.name}</a>`).join(``);if(t&&e[t]){let n=e[t];r.innerHTML=`
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
              ...
              <div class="space-y-6">
                ${Object.entries(n).map(([e,t])=>`
                  <div>
                    <h3 class="text-base font-medium text-gray-800 mb-2">${t.icon} ${e} — ${t.desc}</h3>
                      ...
                      ${t.agents.map(e=>`
                        <div class="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-lg text-sm">
                          ...
                          <div>
                            ...
                            <div class="text-gray-500 text-xs">${e.role}</div>
                          </div>
                        </div>
                      `).join(``)}
                    </div>
...
Report Generation</pre>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ${Object.entries(e).filter(([e])=>e!==`SPEC.md`).map(([e,t])=>`
                  <a href="?doc=${e}" class="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                    <span class="text-2xl">${t.icon}</span>
                    <div>
                      <div class="font-medium text-gray-900">${t.name}</div>
                      <div class="text-gray-500 text-xs">${e}</div>
                    </div>
                  </a>
                `).join(``)}
              </div>
